      ...
      {props.data.map((item, index) => {
        ...
        return (
          <>
            ...
            >
              <h2 className="text-base text-gray-500 inter">{item.departments[0].name}</h2>
              <h1 className="text-2xl">{item.title}</h1>
            </AccordionItem>
            <AccordionPanel id={`panel-${index + 1}`}>
              <div className="inter">
                <p className="text-sm px-6 py-4 overflow-scroll">{parse(decode(item.content))}</p>
                <a
                  target="_blank"
...
const style = {
  item: `block focus:outline-none my-2 p-3`,
